...
describe('generateVoipCalls', () => {
  ...
  it('keeps every event within [start, endDate] — no future calls', () => {
    // Run a large pass to exercise the daysAgo === 0 branch many times: that
    // branch used to produce a call after endDate (business hour drawn in the
    // afternoon while endDate is at noon) → flaky upper-bound failure. The
    // generator now clamps to endDate, so the bound is STRICT here — this test
    // is the regression guard (cf todo/2026-06-02-flaky-voip-calls-spec-line-86.md).
    const events = generateVoipCalls({ ...baseConfig, callCount: 500 });
