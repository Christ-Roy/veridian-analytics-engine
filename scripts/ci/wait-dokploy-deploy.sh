...
#
# → Le seul juge fiable que la prod est saine = une CONFIRMATION FONCTIONNELLE
#   STABLE de l'application elle-même, en loopback DANS le container prod (bypass
#   Traefik/DNS qui flottent depuis un runner). On combine donc :
#     a. on poll le statut Dokploy (signal rapide quand tout va bien) ;
#     b. si Dokploy dit `done` → on confirme quand même par 1 health 200 ;
#     c. si Dokploy dit `error`/`timeout` → on NE conclut PAS échec : on laisse
#        un grace fonctionnel de 5 min et on exige 5 health 200 CONSÉCUTIFS
#        (status:ok + clickhouse:ok + champ version présent) avant de trancher.
#        Échec ROUGE (FAILED → rollback) UNIQUEMENT si la prod reste cassée
#        après ce grace (health 5xx/000 persistant) — jamais sur un Dokploy-error
#        menteur seul.
#
# NB : on confirme l'app fonctionnelle (health stable + clickhouse:ok + version),
#      pas le SHA exact du code servi. Un grep du symbole/SHA du commit dans le
#      code servi (`docker exec <engine> grep -rl <marqueur> /app/dist`) serait
#      une preuve PLUS forte que LE BON code est déployé (le champ version n'est
#      pas bumpé sur les commits fix-only) — c'est ce qu'on fait en validation
#      manuelle de promo. Ici on se limite à la santé fonctionnelle stable, qui
#      suffit à distinguer "Dokploy a menti" de "prod réellement cassée".
set -uo pipefail
...
# en interrogeant l'engine sur 127.0.0.1:3000 DANS son propre container (pas de
# Traefik/DNS dans le chemin). Renvoie 0 si la prod est fonctionnelle et stable,
# 1 sinon. Paramètres : $1 = nb de 200 consécutifs requis, $2 = nb max d'essais.
confirm_app_healthy() {
  local need="$1" max_tries="$2" streak=0 i body
  ...
  for i in $(seq 1 "$max_tries"); do
    ...
    if echo "$body" | grep -q '"status":"ok"' \
      ...
      streak=$((streak + 1))
      echo "  [confirm] health OK ($streak/$need) : ${body:0:80}" >&2
      [ "$streak" -ge "$need" ] && return 0
    else
      [ "$streak" -gt 0 ] && echo "  [confirm] health KO → streak reset" >&2
      streak=0
    fi
    sleep 6
  done
...
echo "[verdict] Dokploy status=$DOK_STATUS — grace fonctionnel 5 min (5×200 consécutifs requis)" >&2
if confirm_app_healthy 5 50; then
  echo "::warning::Dokploy a rapporté '$DOK_STATUS' mais la prod sert un health 200 stable (5 consécutifs, clickhouse:ok, version présente) → deploy traité comme RÉUSSI (statut Dokploy non fiable pendant le recreate). Pas de rollback." >&2
  echo DONE; exit 0
fi
echo "::error::Dokploy=$DOK_STATUS ET prod sans health 200 stable après 5 min → deploy réellement cassé" >&2
echo FAILED; exit 0
