 ...
 *
 * Les credentials OVH (`OvhCreds` de U8) ne portent QUE
 * `{ applicationKey, applicationSecret, consumerKey, endpoint }` — pas de
 * billingAccount ni de serviceName. B-VOIP les DÉCOUVRE depuis l'API OVH :
 *
 ...
 *
 * → Zéro champ supplémentaire à demander au tenant : la saisie U8
 * (3 clés OVH) suffit à pull TOUS les call logs de TOUTES ses lignes.
 *
 * Auth OVH : signature
...
import type { OvhCreds } from "../../credentials/providers.js";
import {
  VoipApiError,
  type CallDirection,
  type CallStatus,
...
/** endpoint OVH → host API (mêmes valeurs que `src/credentials/providers.ts`). */
